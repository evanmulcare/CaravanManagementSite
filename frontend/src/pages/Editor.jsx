  ...
  const smallArr = data.map((data, index) => {
    return(
      ...
      <div className='flex items-center space-x-2 text-sm'>
        ...
        <div>
        <span className='p-2 text-md font-medium tracking-wider text-black bg-yellow-200 rounded-lg bg-opacity-50 flex justify-around'>
            <Link className='hover:underline' to={`/blog/${data._id}`}><GrView /></Link>
            <RxDividerVertical />
            <button onClick={(e) => blogDelete(data._id, e)} className='hover:underline'><FaRegEdit /></button>
          </span> 
        </div>
